...
const WhiteBitFutures = ({ ticker }: IOrderBookProps) => {
  ...
  useEffect(() => {
    ...
    socket.onopen = () => {
      const subscribe = {
        id: 12,
        method: "futures_depth_subscribe",
        params: [`${ticker}_USDT`, 5, "1", true],
      };
    ...
    //   if (params) {
    //     if (params[1].asks) {
    //       setBuy(params[1].asks[0]);
    //     }
    //     if (params[1].bids) {
    //       setSell(params[1].bids[0]);
    //     }
    //   }
    // };
...
export default WhiteBitFutures;
